...
import {RequestAnimationFrame, Vector3D, Quaternion}												from "awayjs-full/lib/core";
import {DefaultRenderer}													from "awayjs-full/lib/renderer";
import {ElementsType}														from "awayjs-full/lib/graphics";
import {MethodMaterial}														from "awayjs-full/lib/materials";
import {Sampler2D}															from "awayjs-full/lib/graphics";
...
class AmmoBasicWorker
{
	...
	private startUP()
	{
		...
		var nextPhysicsWorker = new Worker('assets/ammo_basic_worker.js');
		if (physicsWorker) physicsWorker.terminate();
		physicsWorker = nextPhysicsWorker;
		nextPhysicsWorker = null;
		if (!physicsWorker) physicsWorker = new Worker('assets/ammo_basic_worker.js');
